...
# --- Extra hidden checks for catching "almost correct" implementations ---

# Stricter tolerances for special property checks (shift invariance, normalization)
_STRICT_TOL = {
    np.float32: dict(rtol=8e-4, atol=2e-5, sum_rtol=5e-4, sum_atol=1e-4),
    np.float16: dict(rtol=1e-2, atol=2e-3, sum_rtol=1e-2, sum_atol=2e-3),
}
...
def _check_extra_hidden(user_impl, logits: np.ndarray, y_logsm: np.ndarray,
    ...
    # 1) Конечные значения (дополнительная проверка)
    if not np.isfinite(y_logsm).all():
        problems.append("log_softmax содержит не-конечные значения (isfinite check)")
    ...
    # Это критическое свойство, которое часто нарушают нестабильные реализации
    c = np.array(100.0, dtype=dtype)
    try:
        ...
        if not np.allclose(y_logsm.astype(np.float64), y_shift.astype(np.float64), 
            ...
            problems.append("нарушена инвариантность к добавлению константы (shift invariance)")
    except Exception as e:
        problems.append(f"ошибка при проверке инвариантности к сдвигу: {e}")
    
    # 5) Нормировка: exp(log_softmax) суммируется в 1
    try:
        ...
        if not np.allclose(s, 1.0, rtol=tol["sum_rtol"], atol=tol["sum_atol"]):
            problems.append("exp(log_softmax) не суммируется в 1 по оси классов (normalization)")
    except Exception as e:
        problems.append(f"ошибка при проверке нормировки: {e}")
    
    # 6) CE эквивалентность: CE == -log_softmax[target]
    try:
        if logits.ndim == 1:
            # 1D: CE должен быть скаляром
            ...
            if isinstance(ce, np.ndarray):
                ...
                if not np.allclose(float(ce), float(ref), rtol=tol["rtol"], atol=tol["atol"]):
                    problems.append("cross_entropy != -log_softmax[target] (1D equivalence)")
        else:
            # 2D: допускается либо вектор per-sample формы (B,), либо скаляр-среднее
            idx = target_idx
            ...
            for sigma in sigmas:
                for shift in [0, 500, -500, 800, -800, 1000, -1000]:
                    for dtype_str in ["float32", "float16"]:
                        ...
                        # Decide visibility: ~10% visible, 90% hidden
...
def _run_test_case(test_case: TestCase, user_impl) -> Tuple[bool, str]:
    ...
    with warnings.catch_warnings():
        ...
        try:
            ...
            # Extra hidden checks (catch "almost correct" implementations)
            # Apply only to hidden tests to avoid giving hints during development
            if not test_case.is_visible:
                if test_case.func == "log_softmax":
                    logits = test_case.input_data["x"]
                    y_logsm = result
                    # Generate dummy target for cross_entropy equivalence check
                    if logits.ndim == 1:
                        target_idx = 0
                    else:
                    ...
                    try:
                        ...
                        # If log_softmax fails, skip extra checks (already caught by main test)
                        pass
